...
Single-threaded version: model.Threads = 1 (no parallelism)
...
class PortfolioMIP:
    ...
    def __init__(self, v, b, r, sym_options=None, verbose=True):
        ...
        if self.verbose:
            ...
            print(f"Solver: Gurobi MIP (single-threaded)")
    ...
    def solve(self, time_limit=3600):
        """Solve the MIP model (single-threaded: Threads = 1)"""
        ...
        self.model.setParam('Threads', 1)   # Use only 1 worker thread
        # ─────────────────────────────────────────────────────────────
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='MIP-based Portfolio Optimization solver (single-threaded, Threads=1)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python Opd_mip_gurobi_1thread.py --input input/small
  python Opd_mip_gurobi_1thread.py --input input/small/test1.txt --timeout 300
  python Opd_mip_gurobi_1thread.py --input input/medium --quiet
        """
    )
