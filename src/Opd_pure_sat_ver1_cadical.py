...
class OpdSAT:
    ...
    def binary_search_optimize(self, timeout=120):
        ...
        while lb <= ub:
            ...
            status, matrix, solve_time = self.solve_with_max_overlap(mid, remaining_time)
...
def _solve_worker(v, b, r, quiet, queue):
    ...
    try:
        ...
        optimal_lambda, matrix, total_time = solver.binary_search_optimize(timeout=999999) # Timeout handled by process
        queue.put({
...
def solve_from_file(filepath, timeout=120, overall_timeout=600, quiet=False):
    ...
    if not queue.empty():
        ...
        result_data = {
            ...
            'Time (s)': round(total_time, 3), # Use solver's internal time measure
